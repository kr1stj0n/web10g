...
struct hull_vars {
	...
	u64 markth64;		/* marking threshold as u64 */
...
static void hull_params_init(struct hull_params *params)
{
	...
	params->drate  = 12500000U;	/* 100Mbps draining rate */
			      ...
			      struct sk_buff **to_free)
{
	 ...
	 */
	psched_tdiff_t delta = psched_get_time() - q->vars.last;
	max_bytes *= q->params.drate / MSEC_PER_SEC;
	max_bytes *= PSCHED_TICKS2NS(delta);
	max_bytes /= NSEC_PER_MSEC;
	...
	if (q->vars.counter > q->vars.markth64) {
		       ...
		       struct netlink_ext_ack *extack)
{
	...
	if (tb[TCA_HULL_MARKTH]) {
		u32 markth = nla_get_u32(tb[TCA_HULL_MARKTH]);
		q->params.markth = markth;
		q->vars.markth64 = (u64)markth;
	}
